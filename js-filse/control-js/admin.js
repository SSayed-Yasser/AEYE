    ...
    querySnapshot.forEach((docSnap) => {
      ...
      const orderHTML = `
        <div class="order-card">
          <h3 class="orderfrom">Order from ${order.user.name}</h3>
...
// Admin credentials
const adminEmail2 = "sayedyassersy1@gmail.com";
const adminEmail12 = "rawanahmedd30@gmail.com";

const adminEmail123 = "seifessam828@gmail.com";

function checkAdminAccess2() {
  const userData = JSON.parse(localStorage.getItem('userData'));
  const adminLink = document.querySelector('a[href="control.html"]');

  if (!userData || !userData.email) {
    showErrorMessage("No user is logged in. Please log in first.");
    document.getElementById('sayeds').style.display = 'none';
    return;
  }

  // Check if the logged-in user is an admin
  if (userData.email === adminEmail2 || userData.email === adminEmail12 || userData.email === adminEmail123) {
    document.getElementById('sayeds').style.display = 'block';
  } else {
    document.getElementById('sayeds').style.display = 'none';
  }
}
...
window.onload = checkAdminAccess2;
