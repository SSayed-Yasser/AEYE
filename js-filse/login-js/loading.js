...
const imageUpload = document.getElementById('imageUpload');
                    ...
                    reader.onload = async () => {
                      try {
                        ...
                        const { data: { text } } = await Tesseract.recognize(
                          base64Image,
                          'eng', // اللغة الافتراضية
                          {
                            logger: info => {
                              if (info.status === 'recognizing text') {
                                progressBar.value = info.progress * 100; // تحديث مؤشر التقدم
                              }
                            },
                            tessedit_char_whitelist: '-+.٠١٢٣٤٥٦٧٨٩ODOSSPHCYLAXIS', // تحسين الدقة بالتركيز على الأحرف المهمة
                            tessedit_pageseg_mode: '7', // الوضع المخصص لخط اليد
                          }
                        );
                ...
                function processAndSavePrescription(text) {
                  ...
                  // إضافة القيم إلى الحقول الموجودة
                  if (odMatch) {
